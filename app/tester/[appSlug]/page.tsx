...
function AppReviewContent() {
  ...
  const handleSubmit = async (review: Omit<Review, 'id' | 'date_tested' | 'created_at'>) => {
    try {
      ...
      if (response.ok) {
        ...
        if (nextApp) {
          setTimeout(() => {
            setToast({ message: `Great! Next up: ${nextApp.name}`, type: 'success' })
            setTimeout(() => {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="mb-6">
        <button onClick={() => router.push('/tester')} className="text-slate-400 hover:text-white transition-colors">
          ← Back to Apps
        </button>
      </div>
