    ...
    <div className="max-w-4xl mx-auto">
      ...
      {reviews.length === 0 ? (
        <div className="text-center py-12 bg-slate-800 border border-slate-700 rounded-lg">
          <p className="text-slate-400">No reviews yet for this app</p>
        </div>
      ) : (
          ...
          {reviews.map((review) => (
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <div>
                  <Link
                    href={`/dashboard/tester/${encodeURIComponent(review.telegram_handle)}`}
                    className="font-semibold text-white hover:text-blue-400 transition-colors"
                  >
                    {review.tester_name}
                  </Link>
                  <p className="text-sm text-slate-400">{review.telegram_handle}</p>
